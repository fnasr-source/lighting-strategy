...
async function runApolloEnrichment(apollo, options = {}) {
  ...
  const segmentDefs = apolloSegmentQueries();
  const accepted = [];
  const nurture = [];
...
async function runApplyPlan(instantly, options = {}) {
  ...
  const targetDailyLimits = {
    [existingSecurity.id]: 30,
    [educatorsCampaignId]: 8,
    [ctfV2CampaignId]: 50,
    [securityConsultantsCampaignId]: 25,
    [securityTrainingLeadersCampaignId]: 20,
    [existingCtf.id]: 0,
  };
...
async function main() {
  ...
  node scripts/simlabs_outreach_manager.mjs enrich [--target-per-segment=500]
