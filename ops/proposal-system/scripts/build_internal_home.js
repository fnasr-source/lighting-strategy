...
function main() {
  ...
  const linkMapPath2 = path.join(root, 'ops', 'dashboards', 'system', 'link-map.json');
  ensureDir(path.dirname(homePath));
  ensureDir(path.dirname(systemPagePath));
  ensureDir(path.dirname(linkMapPath2));
  fs.writeFileSync(homePath, homeHtml, 'utf8');
  fs.writeFileSync(systemPagePath, systemHtml, 'utf8');
  fs.writeFileSync(linkMapPath2, JSON.stringify(linkMap, null, 2) + '\n', 'utf8');
