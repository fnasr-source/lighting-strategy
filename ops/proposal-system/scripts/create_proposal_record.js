...
function main() {
  ...
  const outgoingFolder = `ops/proposal-system/_Outgoing/${proposalNumber}`;
  ...
  if (!skipHubBuild) {
    const scripts = [
      path.join(root, 'ops', 'proposal-system', 'scripts', 'build_proposals_hub.js'),
      path.join(root, 'ops', 'proposal-system', 'scripts', 'build_strategies_hub.js'),
      path.join(root, 'ops', 'proposal-system', 'scripts', 'build_internal_home.js')
    ];
